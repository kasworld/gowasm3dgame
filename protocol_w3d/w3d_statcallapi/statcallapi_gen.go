...
type StatCallAPI [w3d_idcmd.CommandID_Count]StatRow

func New() *StatCallAPI {
	cps := new(StatCallAPI)
	for i := 0; i < w3d_idcmd.CommandID_Count; i++ {
		cps[i].Name = w3d_idcmd.CommandID(i).String()
	}
	return cps
}
func (cps *StatCallAPI) BeforeSendReq(header w3d_packet.Header) (*statObj, error) {
	if int(header.Cmd) >= w3d_idcmd.CommandID_Count {
		return nil, fmt.Errorf("CommandID out of range %v %v",
			header, w3d_idcmd.CommandID_Count)
	}
	return cps[header.Cmd].open(), nil
}
func (cps *StatCallAPI) AfterSendReq(header w3d_packet.Header) error {
	if int(header.Cmd) >= w3d_idcmd.CommandID_Count {
		return fmt.Errorf("CommandID out of range %v %v", header, w3d_idcmd.CommandID_Count)
	}
	n := int(header.BodyLen()) + w3d_packet.HeaderLen
	cps[header.Cmd].addTx(n)
	return nil
}
func (cps *StatCallAPI) AfterRecvRsp(header w3d_packet.Header) error {
	if int(header.Cmd) >= w3d_idcmd.CommandID_Count {
		return fmt.Errorf("CommandID out of range %v %v", header, w3d_idcmd.CommandID_Count)
	}
	n := int(header.BodyLen()) + w3d_packet.HeaderLen
