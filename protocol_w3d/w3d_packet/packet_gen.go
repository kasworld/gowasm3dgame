...
const (
	...
	MaxPacketLen = HeaderLen + w3d_const.MaxBodyLen
)
...
func (h Header) String() string {
	...
	case Request:
		return fmt.Sprintf(
			"Header[%v:%v ID:%v Error:%v BodyLen:%v BodyType:%v Fill:%v]",
			h.FlowType, w3d_idcmd.CommandID(h.Cmd), h.ID, h.ErrorCode, h.bodyLen, h.bodyType, h.Fill)
	case Response:
		return fmt.Sprintf(
			"Header[%v:%v ID:%v Error:%v BodyLen:%v BodyType:%v Fill:%v]",
			h.FlowType, w3d_idcmd.CommandID(h.Cmd), h.ID, h.ErrorCode, h.bodyLen, h.bodyType, h.Fill)
	case Notification:
		return fmt.Sprintf(
			"Header[%v:%v ID:%v Error:%v BodyLen:%v BodyType:%v Fill:%v]",
			h.FlowType, w3d_idnoti.NotiID(h.Cmd), h.ID, h.ErrorCode, h.bodyLen, h.bodyType, h.Fill)
	}
}
...
type Header struct {
	...
	ErrorCode w3d_error.ErrorCode // sender set, Response error
...
func MakeHeaderFromBytes(buf []byte) Header {
	...
	h.ErrorCode = w3d_error.ErrorCode(binary.LittleEndian.Uint16(buf[10:12]))
...
func Packet2Bytes(pk *Packet,
	...
	if bodyLen > w3d_const.MaxBodyLen {
		return nil,
