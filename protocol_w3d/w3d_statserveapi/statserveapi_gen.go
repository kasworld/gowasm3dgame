...
type StatServeAPI [w3d_idcmd.CommandID_Count]StatRow

func New() *StatServeAPI {
	ps := new(StatServeAPI)
	for i := 0; i < w3d_idcmd.CommandID_Count; i++ {
		ps[i].Name = w3d_idcmd.CommandID(i).String()
	}
	return ps
}
func (ps *StatServeAPI) AfterRecvReqHeader(header w3d_packet.Header) (*StatObj, error) {
	if int(header.Cmd) >= w3d_idcmd.CommandID_Count {
		return nil, fmt.Errorf("CommandID out of range %v %v", header, w3d_idcmd.CommandID_Count)
	}
...
func (sm *StatObj) AfterSendRsp(hd w3d_packet.Header) {
...
func (sr *StatRow) open(hd w3d_packet.Header) *StatObj {
	sr.mutex.Lock()
	defer sr.mutex.Unlock()
	sr.RecvCount++
	rxbyte := int(hd.BodyLen()) + w3d_packet.HeaderLen
	sr.RecvBytes += rxbyte
...
func (sr *StatRow) afterSend(diffDur time.Duration, hd w3d_packet.Header) {
	sr.mutex.Lock()
	defer sr.mutex.Unlock()
	sr.SendCount++
	txbyte := int(hd.BodyLen()) + w3d_packet.HeaderLen
	sr.SendBytes += txbyte
